...
def test_scrape_concurrency_get_payloads_and_filtering(monkeypatch):
    ...
    # _concurrent_scraper with mapping
    ...
    # scrape_data filtering branches
    rows = [
...
def test_scrape_uncovered_branches_for_table_result_and_filter_append(monkeypatch):
    ...
    # Line 64: no table found branch.
    monkeypatch.setattr(scrape, "urlopen", lambda req, timeout=10: DummyResponse(b"<html><body></body></html>"))
    assert scrape._fetch_table_page(1) == []

    # Line 78: append tmp_row when a new record begins and tmp_row already has data.
    ...
    # Line 152: missing <dd> for one of the expected indices triggers `continue`.
    ...
    # Line 310: filtered_rows.append(row) branch in scrape_data filtering path.
